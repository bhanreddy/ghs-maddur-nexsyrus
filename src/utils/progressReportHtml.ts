...
function card(
  ...
  return `<article class="report-card ${reportType}">
    ...
    <header class="school-header">
      ...
      <div class="copy-label">${html(copyLabel)}</div>
    </header>
    <div class="title-row">
      <div>
...
function stylesheet(brand: ProgressReportBrand) {
  return `<style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    ...
    .page { height: 285mm; display: grid; grid-template-rows: repeat(2, minmax(0, 1fr)); gap: 4mm; break-after: page; page-break-after: always; }
    ...
    .affiliation { color: ${brand.secondary}; font-size: 5.7pt; line-height: 1.3; font-weight: 800; letter-spacing: .35pt; text-transform: uppercase; }
    .contact { color: #596579; font-size: 5.5pt; line-height: 1.3; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .copy-label { justify-self: end; align-self: start; margin-top: 2mm; color: #8792a5; font-size: 5.2pt; font-weight: 800; letter-spacing: .4pt; text-transform: uppercase; }
    .title-row { min-height: 12mm; display: flex; align-items: center; justify-content: space-between; gap: 4mm; padding: 1.7mm .8mm 1.5mm; }
    .report-kind { color: ${brand.secondary}; font-size: 5.6pt; line-height: 1.2; font-weight: 900; letter-spacing: .65pt; text-transform: uppercase; }
    .report-title { color: #172033; font-size: 9pt; line-height: 1.2; font-weight: 900; text-transform: uppercase; }
    .year { text-align: right; font-size: 7pt; font-weight: 800; white-space: nowrap; }
    .year span { display: block; color: #7b879b; font-size: 4.7pt; line-height: 1.2; letter-spacing: .35pt; text-transform: uppercase; }
    ...
    .student-info span, .summary span { display: block; color: #7a879a; font-size: 4.5pt; line-height: 1.1; font-weight: 800; letter-spacing: .35pt; text-transform: uppercase; }
    .student-info strong { display: block; overflow: hidden; color: #1e293b; font-size: 6.7pt; line-height: 1.4; font-weight: 800; white-space: nowrap; text-overflow: ellipsis; }
    table { width: 100%; border-collapse: collapse; table-layout: fixed; margin: 0; }
    th { height: 6mm; padding: .7mm .8mm; background: ${brand.primary}; border: .18mm solid rgba(255,255,255,.2); color: #fff; font-size: 5.3pt; line-height: 1.05; font-weight: 800; text-align: center; }
    th small { display: block; opacity: .8; font-size: 4.3pt; }
    td { height: 4.45mm; padding: .5mm .8mm; border: .2mm solid #d6deea; color: #263247; font-size: 5.65pt; line-height: 1.1; font-weight: 650; text-align: center; }
    ...
    .summary strong { color: #172033; font-size: 6.5pt; line-height: 1.3; font-weight: 900; }
    .summary .result { font-size: 5.5pt; letter-spacing: .3pt; }
    .result.pass { color: #087f5b; } .result.pending { color: #a16207; } .result.support { color: #b42318; }
    .strength { height: 6.5mm; display: flex; align-items: center; gap: 2mm; padding: 1.1mm .8mm 0; color: #596579; font-size: 5.4pt; font-weight: 800; }
    .strength div { flex: 1; height: 3mm; border-bottom: .2mm solid #9aa6b7; }
    footer { display: grid; grid-template-columns: 1fr 1.25fr 1fr 1fr; gap: 5mm; align-items: end; padding: .8mm 2mm 0; color: #536075; font-size: 5.1pt; font-weight: 800; text-align: center; }
    footer > div:not(.motto) i { display: block; height: 4.5mm; border-bottom: .2mm solid #7b8797; margin-bottom: .8mm; }
    footer .motto { align-self: center; color: ${brand.secondary}; font-size: 5.2pt; font-style: italic; }
...
export function buildTwoUpProgressReportHtml(
  ...
  const records = students.length === 1 && options?.duplicateSingle
  ...
  for (let index = 0; index < records.length; index += 2) {
    ...
    const duplicate = records.length === 2 && records[0].id === records[1].id;
    pages.push(`<section class="page">
      ${card(first, reportType, brand, duplicate ? 'School copy' : '')}
      ${second ? card(second, reportType, brand, duplicate ? 'Parent copy' : '') : '<div class="empty-slot"></div>'}
    </section>`);
  }
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${html(reportType === 'component' ? 'Component-Based' : 'Direct')} Assessment Progress Reports</title>${stylesheet(brand)}</head><body>${pages.join('')}</body></html>`;
}
